...
class ShopState extends MusicBeatState //laziness lololol
{
	...
	var optionShit:Array<String> = ['picardo'];
	...
	override function create()
	{
		...
		for (i in 0...optionShit.length)
		{
			...
			menuItem.frames = Paths.getSparrowAtlas('shop/menu_' + optionShit[i]);
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			if (controls.ACCEPT)
			{
				if (optionShit[curSelected] == 'picardo')
				{
					if (FlxG.save.data.quidzz > 1515)
					{
						FlxG.save.data.picardobought = true;
						if (FlxG.save.data.picardobought == true){
							PlayState.SONG = Song.loadFromJson('rofl-hard', 'rofl');
					...
					{
						FlxG.sound.play(Paths.sound('cantbuy'));
					}
				}
				else
				{
